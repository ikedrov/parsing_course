# Обработать все теги <p> в предоставленной HTML-строке.
# Для каждого тега <p>, проверить длину текста внутри тега без учета пробелов. Если длина текста чётная, продолжить обработку.
# Извлечь значения атрибутов id и class из соответствующих тегов. Предполагается, что значения этих атрибутов представляют собой целые числа.
...
def sum_even_length_ids():
    total_id_sum = 0
    total_class_sum = 0
    soup = BeautifulSoup(html, 'html.parser')
    all_tags_p = soup.find_all('p')
    for tag in all_tags_p:
        if len(tag.text.replace(' ', '')) % 2 == 0:
            total_id_sum += int(tag.get('id'))
            total_class_sum += int(*tag.get('class'))
