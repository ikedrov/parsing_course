...
def calculate_total_price(html: str):

    soup = BeautifulSoup(html, 'html.parser')

    all_prices_list = [i.text for i in soup.find_all('p', class_='count price')]
    all_counts_list = [i.text for i in soup.find_all('p', class_='count stock')]

    total = sum(float(p.split(': ')[-1].replace('$', '')) * (int(c.split(': ')[-1])) for p, c
                in zip(all_prices_list, all_counts_list))
