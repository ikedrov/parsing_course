...
# # Обновление страницы
# driver.refresh()
# Анализ и Вербовка: Пройдитесь по всем кукам и определите самого молодого и перспективного хакера — того, кто знает больше всего языков программирования.
...
for cookie in cookies:
    ...
    count_language = len(driver.find_element(By.ID, 'skillsList').find_elements(By.TAG_NAME, 'li'))

    results.append({
        'age': age,
        'count_language': count_language,
        'cookie': [x.get('value', None) for x in driver.get_cookies()],
    })
...
sorted_results = sorted(results, key=lambda x: (x['age'], -x['count_language']))
